  ...
  const content = (
    <>
      <div className="shrink-0 relative w-12 h-12 rounded-full overflow-hidden border border-gray-300">
        <Image src={donationImage ?? "/defaultFeedImage.png"} alt={name} fill />
      </div>
      <div className="w-38 flex-1 flex gap-0.5 flex-col">
        <p
          className={`${type === "tab" || type === "summary" ? "text-gray-400 text-sm" : "truncate font-medium"}`}
        >
          {type === "tab" ? createdAt : name}
        </p>
        <div className=" font-medium flex justify-between items-center">
          <p
            className={`flex-1 truncate ${userName ? "" : "text-sm text-gray-400 center"}`}
          >
            {userName ?? createdAt}
          </p>
