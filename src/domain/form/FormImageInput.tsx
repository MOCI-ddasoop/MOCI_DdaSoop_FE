...
export default function FormImageInput({ images, onChangeImages }: FormImageInputProps) {
  ...
  const emptySlots = previews.length === 0 ? 3 : Math.max(0, MAX_IMAGES - previews.length);
  const slots = [
    ...previews,
  ...
  const handleAddImages = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if(totalImages > MAX_IMAGES) {
      alert(`최대 ${MAX_IMAGES}장까지만 업로드할 수 있어요.`);
      e.target.value = "";
      return;
    }
